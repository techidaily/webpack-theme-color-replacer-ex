...
class AssetsExtractor {
  ...
  extractAssets(assets) {
    ...
    if (0) {
      ...
      fs.writeFileSync(fileName, [].concat(this.extractor.getTempContents()).join('\n'.repeat(2)))
    }

    return output
  }

  extractToArray(assets) {
    var srcArray = extractAll(this)
    if ((srcArray.length === 0 && !this._uglyChanged)) {
      // 容错一次
      this._uglyChanged = true
      this.options.isJsUgly = !this.options.isJsUgly
      //清空缓存
      Object.keys(assets).map(fn => assets[fn]._themeCssCache[this.options.key] = 0)
    ...
    function extractAll(that) {
      ...
      Object.keys(assets).map(fn => {
        ...
        var asset = assets[fn]
        asset._themeCssCache = asset._themeCssCache || {}
        var cssRules = asset._themeCssCache[that.options.key] || that.extractAsset(fn, asset)
        asset._themeCssCache[that.options.key] = cssRules
  ...
  extractAsset(fn, asset) {
    if (fn.match(CssExtReg)) {
      ...
      writeFileForDebugIf(fn, src, this.extractor)
      var cssSrcs = []
      var CssCodeReg = this.options.isJsUgly ? Css_Loader_Reg_UGLY : Css_Loader_Reg_DEV;
      ...
      if (process.env.npm_config_theme_debug) {
        try {
          if (extractor.testCssCode(src)) {
            var info = '/*var theme_cfg=' + JSON.stringify(this.options) + '*/\n' + src
...
module.exports = AssetsExtractor
